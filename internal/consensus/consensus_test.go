...
type mockTmux struct {
	...
	// paneLinesAfterWake overrides paneLines after WakePane is called for a session.
	// Used to simulate the session being busy after prompt dispatch (for timeout tests).
	paneLinesAfterWake map[string][]string
}
...
func newMockTmux() *mockTmux {
	return &mockTmux{
		...
		paneLinesAfterWake: make(map[string][]string),
	}
}
...
func (m *mockTmux) CapturePaneLines(session string, n int) ([]string, error) {
	...
	// After WakePane, use the "after wake" lines if configured (for timeout tests).
	if m.wakeCalled[session] {
		if lines, ok := m.paneLinesAfterWake[session]; ok {
			if n >= len(lines) {
				return lines, nil
			}
...
func TestRun_Timeout(t *testing.T) {
	mock := newMockTmux()
	// Set up a session that looks idle initially (so it passes pre-flight)...
	mock.paneLines["sess-a"] = []string{"old", "❯ ", "⏵⏵ bypass permissions on (shift+tab)"}
	mock.paneContent["sess-a"] = "old\n❯ "
	mock.postContent["sess-a"] = "old\n❯ \nstill working..."

	// ...but after WakePane (during collection), the session shows busy status,
	// so waitForIdle never sees the idle prompt and times out.
	mock.paneLinesAfterWake["sess-a"] = []string{"processing...", "⏵⏵ bypass permissions on ... · esc to interrupt"}
	...
	if result.Sessions[0].Status != StatusTimeout {
		t.Errorf("expected timeout status, got %s (error: %s)", result.Sessions[0].Status, result.Sessions[0].Error)
	}
}
...
func TestWaitForIdle_Timeout(t *testing.T) {
	mock := newMockTmux()
	// Session never shows the prompt — waitForIdle should time out.
	mock.paneLines["sess"] = []string{"still processing...", "⏵⏵ bypass permissions on ... · esc to interrupt"}

	p := ProviderInfo{Name: "claude", PromptPrefix: "❯ "}
	err := waitForIdle(mock, "sess", p, 300*time.Millisecond)
	if err == nil {

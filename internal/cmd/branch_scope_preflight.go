...
func resolveDefaultBaseRef(g *git.Git) (string, error) {
	...
	return "", fmt.Errorf("branch scope preflight: cannot resolve default base ref (neither origin/main nor origin/master exist)")
}
...
func runBranchScopePreflight(g *git.Git, baseRef string) error {
	...
	if len(changedFiles) == 0 {
		style.PrintWarning("branch scope preflight: zero files changed against %s (possible wrong base ref or botched rebase)", baseRef)
		return nil
	}
